...
def torch2tvm_module(torch_module: torch.nn.Module, torch_inputs: Tuple[torch.Tensor, ...], target):
    ...
    with torch.no_grad():
        ...
        # torch.onnx.export(torch_module, torch_inputs, "super_resolution.onnx", input_names=input_names,
        #                   output_names=["o" + str(i) for i in range(len(torch_inputs))], opset_version=11)
        # outs = torch_module(*torch_inputs)
        ...
        input_shapes = {'input': (1, 24, 224, 224)}
